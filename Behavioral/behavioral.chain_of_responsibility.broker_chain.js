/*Створення запиту:
Коли викликається геттер attack або defense у класі Creature, створюється об'єкт Query,
 який містить поточне значення властивості.

Передача запиту:
Запит передається через метод performQuery класу Game, який викликає всі підписані 
обробники (модифікатори).

Обробка запиту:
Кожен модифікатор (CreatureModifier) перевіряє, чи запит стосується його 
істоти та типу властивості (attack або defense).
Якщо умови виконуються, модифікатор змінює значення query.value.

Результат:
Після проходження через всі модифікатори значення запиту повертається до геттера, 
і модифіковане значення використовується.
*/
